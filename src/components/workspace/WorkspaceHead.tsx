...
import CardsWorkspace from "../cards/WorkSpaceCard"
...
const WorkspaceHead = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await axios.get("http://localhost/myapp/index.php");
    ...
    const delhi = data.filter((item:any)=>item.catagery === "Delhi")
    const chandigarh = data.filter((item:any)=>item.catagery === "Chandigarh")
    const noyda = data.filter((item:any)=>item.catagery === "Noyda")
    const jaipur = data.filter((item:any)=>item.catagery === "Jaipur")
    const kangra = data.filter((item:any)=>item.catagery === "Kangra")
    const dheradun = data.filter((item:any)=>item.catagery === "Dheradun")
    console.log(delhi,"--------------")
    return (
        <>
            <section>
                <div className="container">
                    <div className="flex flex-col md:pt-[60px] pt-[22px] md:pb-[50px] pb-[20px] font-inter">
                        <div className="flex flex-col justify-center items-center md:gap-[50px] gap-[36px] md:pb-[100px] pb-[50px]">
                            ...
                            <div className="flex overflow-hidden bg-whiteGray rounded-xl justify-between items-center max-w-[800px]  w-full md:text-lg text-[10px] leading-[100%]">
