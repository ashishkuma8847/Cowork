...
const Footer = () => {
  return (
            ...
            <div className="container grid grid-cols-1 md:grid-cols-2 gap-[30px] sm:gap-10 lg:gap-[100px]">
                <div className=" ">
                    {
                        footerjson.company?.map((item:any, index:number) => (
                            <div  key={index + Date.now()} className="flex flex-col max-w-[485px] w-full gap-3.5">
                                <div className="flex items-center gap-2.5">
                                    <img src={`/svg/${item.image}`} alt="" />
                ...
                <div className="grid grid-cols-2 md:flex justify-between  max-w-[618px] w-full">
                    {
                        footerjson.sections?.map((item:any, index:number) => (
                            <div  key={index + Date.now()} className="flex flex-col ">
                                ...
                                {item.links.map((item:any, index:number) => (
                                    <h1 key={index} className="font-inter font-normal text-sm sm:text-base leading-[130%] text-darkGray ">
                                        {
                                            item.linkdata
                                        }
                                    </h1>
